...
class AdvancedSearchForm extends React.Component {
  ...
  setRegistration = async (e) => {
    ...
    if (login.trim() == '') regAnswer = 'Логин не должен содержать одни пробелы!!!';
    else if (password.trim() == '') regAnswer = 'Пароль не должен содержать одни пробелы!!!';
    else if (login.length < 8) regAnswer = 'Длина логина должна быть больше 8 символов!!!';
    else if (login.length < 8) regAnswer = 'Длина пароля должна быть больше 8 символов!!!';
    this.setState({ alert_n: regAnswer });
    ...
    if (regAnswer != 'Регистрация успешно завершена!')
    {
      ...
      await this.addUser(login, password)
        ...
        .catch((e) => {
          ...
          this.setState({ alert: "Got fetch error" });
        });        
